    ...
    <div class="login-container">
        <el-form ref="registerForm" :model="registerForm" :rules="registerRules" class="login-form" auto-complete="on"
            ...
            <el-button :loading="loading" type="primary" style="width:100%;margin-bottom:30px;"
                       @click.native.prevent="handleLogin">Register
            </el-button>
            <el-link type="warning" href="/#/login">Already Registered?</el-link>
        </el-form>
    </div>
</template>

<script>
    import {validEmail,confirmPasswordMatch} from '../../utils/validate'
    ...
    export default {
        ...
        data() {
            ...
            const validateName =(rule,value,callback) =>{
                if (value.length < 1) {
                    callback(new Error('Name Can not be less than 1 character'))
                } else {
                    callback()
                }
            }
            ...
            const validatePasswordConfirmation = (rule, value, callback) =>{
                if (!confirmPasswordMatch(value, this.registerForm.password)) {
